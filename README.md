# seawolf82/vsftpd_openjdk1.8_xvfb


This Docker container implements a vsftpd server+openjdk1.8+xvfb, with the following features:
...
### Installation from [Docker registry hub](https://hub.docker.com/r/seawolf82/vsftpd_openjdk1.8_xvfb/).

You can download the image with the following command:

```bash
docker pull seawolf82/vsftpd_openjdk1.8_xvfb
```
...
```bash
  docker run --rm vsftpd seawolf82/vsftpd_openjdk1.8_xvfb
```

2) Create a container in active mode using the default user account, with a binded data directory:

```bash
docker run -d -p 21:21 -v /my/data/directory:/home/vsftpd --name vsftpd seawolf82/vsftp_openjdk_xvfb
...
--name vsftpd --restart=always seawolf82/vsftp_openjdk_xvfb
```
